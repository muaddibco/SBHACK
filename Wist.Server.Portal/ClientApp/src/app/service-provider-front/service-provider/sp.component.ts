...
import { NotificationPopupDialog } from 'src/app/notification-popup/notification-popup.component';
  ...
  initializeSession() {
    ...
    this.serviceProviderService.getServiceProvider(this.route.snapshot.paramMap.get('id')).subscribe(r => {
      ...
      this.serviceProviderService.getSessionInfo(this.route.snapshot.paramMap.get('id')).subscribe(r => {
        ...
        that.qrCodeContent = btoa(that.baseUrl + "SpUsers/GetActionInfo?actionType=0&publicKey=" + r.publicKey + "&sessionKey=" + r.sessionKey);
        that.qrCode2Content = btoa(that.baseUrl + "SpUsers/GetActionInfo?actionType=1&publicKey=" + r.publicKey + "&sessionKey=" + r.sessionKey);
        ...
        that.serviceProviderService.getSpDocuments(that.route.snapshot.paramMap.get('id')).subscribe(r => {
          ...
          for (let doc of r) {
            that.documents.push({
              model: doc,
              qrContent: btoa(that.baseUrl + "SpUsers/GetActionInfo?actionType=2&publicKey=" + that.publicKey + "&sessionKey=" + that.sessionKey + "&registrationKey=" + doc.hash),
